...
var colorShader;
function onCreate(){
	var limoSunset:BGSprite = new BGSprite("stages/limo/erect/limoSunset", -120, -50, 0.1, 0.1);
	...
	bgLimo = new BGSprite("stages/limo/erect/bgLimo", -200, 480, 0.4, 0.4, ["background limo blue"], true);
	...
	for (i in 0...5){
		...
		sprite.animation.addByPrefix("hit1", "hench hit 1",24, false);
		sprite.animation.addByPrefix("hit2", "hench hit 2",24, false);
	...
	var limoDrive = new BGSprite("stages/limo/erect/limoDrive", -128, 528, 1, 1, ["Limo stage"], true);
...
function onCreatePost(){
	...
	for (i in poleLamp) i.shader = colorShader;
	for (i in lampPole) i.shader = colorShader;
	boyfriendLight.shader = boyfriendPole.shader = colorShader;
	for (i in henchmenGroup) i.shader = colorShader;
...
function gibHenchman(target:FlxSprite) {
	...
	legGib.shader = headGib.shader = bloodEffect.shader = armGib.shader = target.shader;
